...
import { useBuyerFurnitureOrders } from "@/lib/hooks/use-buyer";
...
export default function FurnitureOrdersPage() {
  ...
  const { data, isLoading, error } = useBuyerFurnitureOrders({ status: tab === "all" ? undefined : tab });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="heading-2">Pesanan Furniture</h1>
        <p className="text-muted-foreground mt-1">Lacak status pesanan furniture Anda</p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Mulai belanja furniture</p>
        ...
        <div className="space-y-3">
          {orders.map((o) => {
            ...
            const seller = o.expand?.seller;
            return (
              <Link key={o.id} href={`/buyer/furniture/orders/${o.id}`}>
                <Card className="hover:border-primary/50 transition-colors cursor-pointer">
                  <CardContent className="pt-4">
                    <div className="flex items-center gap-4">
                      <div className="h-14 w-14 bg-muted rounded flex items-center justify-center shrink-0">
                        {product?.photos?.[0] ? <img src={getFileUrl("generator_products", product.id, product.photos[0])} alt="" loading="lazy" className="h-full w-full object-cover rounded" /> : <Package className="h-6 w-6 text-muted-foreground" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm truncate">{product?.name || "Produk"}</p>
                        <p className="text-xs text-muted-foreground">
                          {o.quantity} × Rp {o.total_price.toLocaleString("id-ID")}
                          {seller && ` • ${seller.name}`}
                        </p>
                      </div>
