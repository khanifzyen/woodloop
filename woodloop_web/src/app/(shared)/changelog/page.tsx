...
const changelog = [
  {
    ...
    date: "1 Juni 2026",
    added: [
      "Halaman Edit Produk Generator — edit nama, kategori, harga, stok, status, foto",
      "Halaman Detail Produk Generator — photo gallery, lightbox, info lengkap",
      "Halaman Konfirmasi Pesanan setelah checkout",
      "Chat real-time — icon chat + unread badge di navbar",
      "Tombol Chat di marketplace — hubungi Supplier langsung dari kartu kayu",
      "Tombol Chat di pesanan — diskusi per order dengan Supplier",
      "Halaman Chat — preview produk/order (bisa dihapus), template pertanyaan (pill button), link referensi di body chat",
      "Mark as read — unread berkurang otomatis saat chat dibuka",
      "Sidebar chat responsive dengan hamburger drawer di mobile",
      "Integrasi Midtrans Snap — tombol Bayar di timber orders, popup pembayaran, webhook notification",
      "Field payment (snap_token, snap_redirect_url, payment_method) di raw_timber_orders",
      "Dashboard Generator — card Pesanan Kayu dengan jumlah real",
      "Halaman Profil Generator — edit nama, telepon, alamat",
      "Wallet di navbar — tampilkan saldo real dari API, link ke /wallet",
      "Halaman Edit Limbah — edit waste listing yang status available",
      "Filter status di halaman Pesanan Kayu Generator",
      "Status produk (Aktif/Draf/Habis) di form tambah produk baru",
      "Service worker auto-unregister di localhost (mencegah cache lama)",
    ...
    fixed: [
      "Race condition hydration — fix error 'Not a generator' saat load halaman",
