...
import '../../../generator/presentation/bloc/raw_timber_order_bloc.dart';
import '../../../auth/presentation/bloc/auth_bloc.dart';
import '../../../../injection_container.dart';

/// Supplier melihat dan mengelola pesanan masuk dari Generator
class SupplierIncomingOrdersPage extends StatelessWidget {
  const SupplierIncomingOrdersPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Pesanan Masuk', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
        centerTitle: true, backgroundColor: Colors.transparent, elevation: 0,
      ),
      body: BlocProvider(
        create: (_) => getIt<RawTimberOrderBloc>()..add(LoadRawTimberOrders(sellerId: userId)),
          ...
          builder: (context, state) {
            ...
            if (state is RawTimberOrdersLoaded) {
              if (state.orders.isEmpty) {
                return const Center(child: Text('Belum ada pesanan masuk', style: TextStyle(color: Colors.white54)));
              }
                  ...
                  return Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      if (o.notes != null && o.notes!.isNotEmpty)
                        Text('Catatan: ${o.notes}', style: const TextStyle(color: Colors.white38, fontSize: 11)),
                      Text(DateFormat('dd MMM yyyy, HH:mm').format(o.created), style: const TextStyle(color: Colors.white38, fontSize: 11)),
                      if (o.status == 'pending') ...[
                        const SizedBox(height: 12),
                        Row(children: [
                          Expanded(child: ElevatedButton(
                            onPressed: () => context.read<RawTimberOrderBloc>().add(UpdateRawTimberOrderStatus(id: o.id, body: {'status': 'accepted'})),
                            ...
                            child: const Text('Terima', style: TextStyle(color: AppTheme.background, fontWeight: FontWeight.bold)),
                          )),
                          const SizedBox(width: 12),
                          Expanded(child: OutlinedButton(
                            onPressed: () => context.read<RawTimberOrderBloc>().add(UpdateRawTimberOrderStatus(id: o.id, body: {'status': 'rejected'})),
